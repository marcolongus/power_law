...
particle create_particle(void) {
	...
	switch(velocity_distribution){
		...
		case(1):
			...
			break;
		case(2):
			velocity = (pow(1 - dis(gen), 1 / (1 - k_alpha))) * v_min;
			break;
		default:
...
particle evolution(vector<particle> &system, vector<int> &index, bool inter){
	...
	if (dis(gen) < p_rotation) {
		KIND ruido = 2 * Pi * dis(gen); // Run & Tumble
		Agent.angle += ruido;
	}


	// Reflective walls:
	bool reflective_walls = false;
	if (reflective_walls) {
	...
	if (Agent.is_infected() && flag && (dis(gen) < p_infection) ) Agent.set_refractary(); // SIR
